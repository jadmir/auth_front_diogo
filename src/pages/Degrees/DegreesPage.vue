...
<script setup>
  ...
  import { DegreeCrudService } from 'src/services/crud/degree.crud.service'
  import DegreeForm from 'pages/Degrees/components/DegreeForm.vue'
  import DegreeTable from 'pages/Degrees/components/DegreeTable.vue'

  const title = 'Grados'
  const description = 'Creación y edición de grados'
  const newItemLabel = 'Nuevo grado'
  const service = new DegreeCrudService()
  const initItem = {
    name: '',
    description: '',
  }
