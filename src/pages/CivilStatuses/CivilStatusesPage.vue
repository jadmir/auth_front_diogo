...
<script setup>
  ...
  import { CivilStatusCrudService } from 'src/services/crud/civilStatus.crud.service'
  import CivilStatusForm from 'pages/CivilStatuses/components/CivilStatusForm.vue'
  import CivilStatusTable from 'pages/CivilStatuses/components/CivilStatusTable.vue'

  const title = 'Estados civiles'
  const description = 'Creación y edición de estados civiles'
  const newItemLabel = 'Nuevo estado civil'
  const service = new CivilStatusCrudService()
  const initItem = {
    name: '',
    description: '',
  }
