...
<script setup>
  ...
  import { SchoolCrudService } from 'src/services/crud/school.crud.service'
  import SchoolForm from 'pages/Schools/components/SchoolForm.vue'
  import SchoolTable from 'pages/Schools/components/SchoolTable.vue'

  const title = 'Escuelas'
  const description = 'Creación y edición de escuelas'
  const newItemLabel = 'Nuevo escuela'
  const service = new SchoolCrudService()
  const initItem = {
    name: '',
    description: '',
  }
