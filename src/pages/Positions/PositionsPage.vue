...
<script setup>
  ...
  import { PositionsCrudService } from 'src/services/crud/positions.crud.service'
  import PositionForm from 'pages/Positions/components/PositionForm.vue'
  import PositionTable from 'pages/Positions/components/PositionTable.vue'

  const title = 'Cargos'
  const description = 'Creación y edición de cargos'
  const newItemLabel = 'Nueva cargo'
  const service = new PositionsCrudService()
  const initItem = {
    name: '',
    description: '',
  }
